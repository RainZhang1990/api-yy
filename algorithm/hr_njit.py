    ...
    def fit(self):
        ...
        while len(cluster_node_dict) > 1:
            ...
            for v in cluster_node_dict.values():
                dist = order_distance(
                    v.vec, new_node.vec, self.np_dist)
    ...
    def merge_order(self, order_list0: list, order_list1: list, target_amount: int):
        ...
        for v in order_list0:
            for v1 in order_list1:
                dist = order_distance(
                    self.order_dict[v], self.order_dict[v1], self.np_dist)
                if dist < n:
                    n = dist
                    order = v1
...
@njit
def order_distance(order0:set, order1:set, np_dist):
    all_dist = Dict.empty(key_type=types.int64, value_type=types.float64)
    for i in order0:
        for j in order1:
            all_dist[i*10**10+j] = np_dist[i, j]
    main_comm0, main_comm1 = {np.int64(-1)},{np.int64(-1)}
    main_dist = main_distance(
    ...
    residue_dist = Dict.empty(
        key_type=types.int64, value_type=types.float64)
    for v in residue_comm:
        for v1 in main_comm:
            residue_dist[v*10**10+v1] = np_dist[v,v1]
    if len(residue_comm)==0:
        return main_dist
    resi_dist = residue_distance(
        residue_dist, residue_comm, np_dist)
    return main_dist + resi_dist


@njit
def main_distance(all_dist, set0, set1, min_len):
    set0.discard(-1)
    set1.discard(-1)
    ...
    set0.add(int(key0))
    set1.add(int(key1))
    return n + main_distance(clear_dict(all_dist, key0, key1), set0, set1, min_len)


@njit
def residue_distance(residue_dist, residue_comm:set, np_dist):
    n = math.inf
    key0 = None
    for k, v in residue_dist.items():
        if v < n:
            n = v
            key0=round(k/10**10)
    new_residue_list(residue_dist, key0, residue_comm, np_dist)
    if len(residue_dist) == 0:
        return 0
    return n + residue_distance(residue_dist, residue_comm, np_dist)


@njit
def new_residue_list(residue_dist, comm, residue_comm:set, np_dist):
    residue_comm.remove(comm)
    clear_dict(residue_dist, comm, comm)
    for k in residue_comm:
        residue_dist[k*10**10+comm] = np_dist[k,int(comm)]
    return residue_dist


@njit
def clear_dict(dictionary, key1, key2):
    for k in list(dictionary.keys())[:]:
        if round(k/10**10) == key1 or k%10**10 == key2:
            del dictionary[k]
    return dictionary
